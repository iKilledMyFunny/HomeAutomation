    ...
    void CySysClkWriteImoFreq(uint32 freq)
    {
        ...
        if ((freq >= CY_SYS_CLK_IMO_MIN_FREQ_MHZ) && (freq <= CY_SYS_CLK_IMO_MAX_FREQ_MHZ))
        {
            ...
            ***************************************************************************/
            ...
            CY_SYS_CLK_IMO_TRIM1_REG = (uint32) (uint8) CY_SFLASH_IMO_TRIM_REG(freq - CY_SYS_CLK_IMO_FREQ_TABLE_OFFSET);
    ...
    *******************************************************************************/
    void CySysClkPllSetSource(uint32 pll, uint32 source)
    {
        ...
        if (pll < CY_IP_PLL_NR)
        {
            ...
            #else
                regTmp = CY_SYS_ECO_CLK_SELECT_REG & (uint32) ~CY_SYS_CLK_SELECT_PLL0_MASK;
                regTmp |= ((source << CY_SYS_CLK_SELECT_PLL0_SHIFT) & CY_SYS_CLK_SELECT_PLL0_MASK);
