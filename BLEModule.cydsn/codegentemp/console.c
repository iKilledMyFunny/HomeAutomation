...
static void console_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void console_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            console_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            console_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (console_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[console_RX_SDA_MOSI_PIN_INDEX] = console_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [console_RX_SDA_MOSI_PIN_INDEX] = console_PIN_DM_OD_LO;
        #elif (console_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[console_RX_WAKE_SDA_MOSI_PIN_INDEX] = console_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [console_RX_WAKE_SDA_MOSI_PIN_INDEX] = console_PIN_DM_OD_LO;
        #else
        #endif /* (console_RX_SDA_MOSI_PIN) */
        
        #if (console_TX_SCL_MISO_PIN)
            hsiomSel[console_TX_SCL_MISO_PIN_INDEX] = console_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [console_TX_SCL_MISO_PIN_INDEX] = console_PIN_DM_OD_LO;
        ...
        {
        #if (console_RX_SDA_MOSI_PIN)
            hsiomSel[console_RX_SDA_MOSI_PIN_INDEX] = console_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (console_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[console_RX_WAKE_SDA_MOSI_PIN_INDEX] = console_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (console_RX_SDA_MOSI_PIN) */
        
        #if (console_TX_SCL_MISO_PIN)
            hsiomSel[console_TX_SCL_MISO_PIN_INDEX] = console_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (console_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[console_RX_SDA_MOSI_PIN_INDEX] = console_PIN_DM_DIG_HIZ;
                pinsDm[console_TX_SCL_MISO_PIN_INDEX] = console_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= console_TX_SCL_MISO_PIN_MASK;
            #endif /* (console_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[console_RX_SDA_MOSI_PIN_INDEX] = console_PIN_DM_STRONG;
                pinsDm[console_TX_SCL_MISO_PIN_INDEX] = console_PIN_DM_DIG_HIZ;
            ...
            #if (console_RX_SDA_MOSI_PIN)
                pinsInBuf |= console_RX_SDA_MOSI_PIN_MASK;
            #elif (console_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= console_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (console_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (console_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (console_TX_SCL_MISO_PIN)
                hsiomSel[console_TX_SCL_MISO_PIN_INDEX] = console_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [console_TX_SCL_MISO_PIN_INDEX] = console_PIN_DM_OD_LO;
            #endif /* (console_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (console_RX_SDA_MOSI_PIN)
                    hsiomSel[console_RX_SDA_MOSI_PIN_INDEX] = console_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [console_RX_SDA_MOSI_PIN_INDEX] = console_PIN_DM_DIG_HIZ;
                #elif (console_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[console_RX_WAKE_SDA_MOSI_PIN_INDEX] = console_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [console_RX_WAKE_SDA_MOSI_PIN_INDEX] = console_PIN_DM_DIG_HIZ;
                #else
                #endif /* (console_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        console_SET_HSIOM_SEL(console_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       console_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[console_RX_SDA_MOSI_PIN_INDEX]);

        console_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[console_RX_SDA_MOSI_PIN_INDEX]);

        #if (!console_CY_SCBIP_V1)
            console_SET_INP_DIS(console_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         console_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & console_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!console_CY_SCBIP_V1) */
        ...
        console_SET_HSIOM_SEL(console_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[console_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        console_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[console_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        console_SET_INP_DIS(console_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     console_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & console_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        console_SET_INCFG_TYPE(console_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        console_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (console_TX_SCL_MISO_PIN)
        console_SET_HSIOM_SEL(console_TX_SCL_MISO_HSIOM_REG,
                                       console_TX_SCL_MISO_HSIOM_MASK,
                                       console_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[console_TX_SCL_MISO_PIN_INDEX]);

        console_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[console_TX_SCL_MISO_PIN_INDEX]);

    #if (!console_CY_SCBIP_V1)
        console_SET_INP_DIS(console_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     console_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & console_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!console_CY_SCBIP_V1) */
    #endif /* (console_RX_SDA_MOSI_PIN) */
