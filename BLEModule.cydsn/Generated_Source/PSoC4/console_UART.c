...
#include "cyapicallbacks.h"
    ...
    * functionality when console_Sleep() function is called:
    * 0 – disable, other values – enable. Default value is 1.
    * It is only available when Enable wakeup from Deep Sleep Mode is enabled.
    */
    uint8 console_skipStart = 1u;
    /** \} globals */
    ...
    ***************************************/
    ...
    const console_UART_INIT_STRUCT console_configUart =
    {
        ...
        (uint8) console_UART_RX_BREAK_WIDTH
    };
    ...
    ****************************************************************************//**
    *
    *  Configures the console for UART operation.
    *
    *  This function is intended specifically to be used when the console
    *  configuration is set to “Unconfigured console” in the customizer.
    *  After initializing the console in UART mode using this function,
    *  the component can be enabled using the console_Start() or
    * console_Enable() function.
    ...
    *******************************************************************************/
    void console_UartInit(const console_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            console_rxBufferSize  =         config->rxBufferSize;

            /* Set TX direction internal variables */
            console_txBuffer      =         config->txBuffer;
            console_txDataBits    = (uint8) config->dataBits;
            console_txBufferSize  =         config->txBufferSize;
            ...
            console_UART_RX_CTRL_REG = console_GET_UART_RX_CTRL_MODE(config->stopBits)              |
                                        ...
                                        console_GET_UART_RX_CTRL_BREAK_WIDTH(config->breakWidth);
            ...
            console_IntrTxMask = LO16(console_INTR_TX_MASK_REG);
...
void console_UartPostEnable(void)
{
        ...
        if (console_CHECK_TX_SCL_MISO_PIN_USED)
        {
            /* Set SCB UART to drive the output pin */
            console_SET_HSIOM_SEL(console_TX_SCL_MISO_HSIOM_REG, console_TX_SCL_MISO_HSIOM_MASK,
                                           console_TX_SCL_MISO_HSIOM_POS, console_TX_SCL_MISO_HSIOM_SEL_UART);
        }
    #endif /* (console_TX_SCL_MISO_PIN_PIN) */
            ...
            if (console_CHECK_RTS_SS0_PIN_USED)
            {
                ...
                console_SET_HSIOM_SEL(console_RTS_SS0_HSIOM_REG, console_RTS_SS0_HSIOM_MASK,
                                               console_RTS_SS0_HSIOM_POS, console_RTS_SS0_HSIOM_SEL_UART);
            }
...
void console_UartStop(void)
{
        ...
        if (console_CHECK_TX_SCL_MISO_PIN_USED)
        {
            ...
            console_SET_HSIOM_SEL(console_TX_SCL_MISO_HSIOM_REG, console_TX_SCL_MISO_HSIOM_MASK,
                                           console_TX_SCL_MISO_HSIOM_POS, console_TX_SCL_MISO_HSIOM_SEL_GPIO);
        }
    #endif /* (console_TX_SCL_MISO_PIN_PIN) */
            ...
            if (console_CHECK_RTS_SS0_PIN_USED)
            {
                ...
                /* Set GPIO to drive output pin */
                console_SET_HSIOM_SEL(console_RTS_SS0_HSIOM_REG, console_RTS_SS0_HSIOM_MASK,
                                               console_RTS_SS0_HSIOM_POS, console_RTS_SS0_HSIOM_SEL_GPIO);
            }
        #endif /* (console_RTS_SS0_PIN) */
    #endif /* !(console_CY_SCBIP_V0 || console_CY_SCBIP_V1) */
        ...
        /* Set GPIO to drive output pin */
        console_SET_HSIOM_SEL(console_TX_HSIOM_REG, console_TX_HSIOM_MASK,
                                       console_TX_HSIOM_POS, console_TX_HSIOM_SEL_GPIO);
    #endif /* (console_UART_TX_PIN) */
        ...
        /* Set GPIO to drive output pin */
        console_SET_HSIOM_SEL(console_RTS_HSIOM_REG, console_RTS_HSIOM_MASK,
                                       console_RTS_HSIOM_POS, console_RTS_HSIOM_SEL_GPIO);
    #endif /* (console_UART_RTS_PIN) */
    ...
    console_IntrTxMask = LO16(console_GetTxInterruptMode() & console_INTR_UART_TX_RESTORE);
}
...
void console_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *   Check console_rxBufferOverflow to capture that error condition.
    *
    *******************************************************************************/
    ...
    *   - console_UART_RX_OVERFLOW - Attempt to write to a full
    *     receiver FIFO.
    *   - console_UART_RX_UNDERFLOW    Attempt to read from an empty
    *     receiver FIFO.
    *   - console_UART_RX_FRAME_ERROR - UART framing error detected.
    *   - console_UART_RX_PARITY_ERROR - UART parity error detected.
    *
    *  \sideeffect
    *   The errors bits may not correspond with reading characters due to
    *   RX FIFO and software buffer usage.
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: console_UartSendBreakBlocking
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void console_UartSendBreakBlocking(uint32 breakWidth)
    {
        ...
        txIntrReg = console_GetTxInterruptMode();
        console_SetTxInterruptMode(0u);
        console_ClearTxInterruptSource(console_INTR_TX_UART_DONE);

        /* Store TX CTRL configuration */
        txCtrlReg = console_TX_CTRL_REG;

        /* Set break width */
        console_TX_CTRL_REG = (console_TX_CTRL_REG & (uint32) ~console_TX_CTRL_DATA_WIDTH_MASK) |
                                        console_GET_TX_CTRL_DATA_WIDTH(breakWidth);

        /* Generate break */
        console_TX_FIFO_WR_REG = 0u;

        /* Wait for break completion */
        while (0u == (console_GetTxInterruptSource() & console_INTR_TX_UART_DONE))
        {
        }
        ...
        console_TX_CTRL_REG = txCtrlReg;
        console_SetTxInterruptMode(txIntrReg);
    }
    ...
    *******************************************************************************/
    void console_UartSaveConfig(void)
    {
        ...
        */
        console_CLEAR_UART_RX_WAKE_INTR;
