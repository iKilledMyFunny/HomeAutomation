...
void put_string(uint8_t *data, uint16_t length)
{
    ...
    console_SpiUartPutArray(data, length);
}
...
// internal vsnprintf
static size_t _vsnprintf(char* buffer, size_t buffer_len, const char* format,
		va_list va)
{
	...
	while (idx < buffer_len)
	{
		...
		case 'F':
		{
			idx += _ftoa((float) va_arg(va, double), &buffer[idx],
					buffer_len - idx, precision, width, flags);
			format++;
			break;
		}
